...
async function carregarTabela() {
  ...
  const resp = await fetch(`/api/asos?${params.toString()}`);
  ...
  const tipos = [...new Set(registros.map(t => t.situacao))];

  container.innerHTML = tipos.map(situacao => {
    const items = registros.filter(t => t.situacao === situacao);
    return `
...
async function editar(id) {
  ...
  const resp = await fetch(`/api/asos/${id}`);
